...
import type { MarketingServicesOverview } from '@/fake-api/marketing-services-overview';
...
export default function MarketingServiceDetailView({ serviceData, overview, listingPath }: Props) {
  return (
    <main className="min-h-screen bg-gray-50">
      {serviceData.cmsSeo?.schema ? (
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: serviceData.cmsSeo.schema }}
        />
      ) : null}
      <CompanyHero
        data={{
                ...
                <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-[#0E233C] mb-4 leading-snug">
                  {serviceData.introSection ? (
                    <>
                      ...
                      <span className="text-[#009FE8]">{serviceData.introSection.headingHighlight}</span>
                    </>
                  ) : (
                    serviceData.title
                  )}
                ...
                >
                  {highlight.icon && (
                    <div className="mb-6 w-12 h-12 flex items-center justify-start">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={highlight.icon} alt="" className="w-full h-full object-contain" />
                    </div>
                  )}
                  <h4 className="text-base md:text-[20px] font-bold text-[#0E233C] mb-2">
                    {highlight.title}
                  </h4>
                  <p className="text-xs md:text-base text-black leading-relaxed">{highlight.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="bg-gray-50 pt-10 md:pt-16">
          <div className="container mx-auto px-4">
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-black mb-8 text-center ">
              Elevate Your{' '}
              <span className="text-[#009FE8]">{serviceData.brandJourney.headingHighlight}</span>
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-5">
              {serviceData.brandJourney.items.map((item) => (
                ...
                >
                  ...
                  <h3 className="text-sm md:text-[20px] font-bold text-[#0E233C] leading-snug">{item.title}</h3>
                  <p className="text-xs md:text-base text-gray-600 mt-1  mb-0 pb-0">{item.subtitle}</p>
                </div>
              ))}
            </div>
          </div>
